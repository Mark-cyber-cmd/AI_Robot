		...
		cmd = UartRxBuffer[3];
 		switch(cmd)
 		{
			...
			case CMD_MULT_SERVO_POS_READ:
				pos_back[0] = 0x55;
			  pos_back[1] = 0x55;
				pos_back[2] = UartRxBuffer[4] * 3 + 3;
				pos_back[3] = 0x15;
			  pos_back[4] = UartRxBuffer[4];
				servoCount = UartRxBuffer[4];
				for(i = 0; i < servoCount; i++)
				{
					id =  UartRxBuffer[5 + i];
					pos_data = BusServoRead(id,SERVO_POS_READ);
					pos_back[5 + i*3] = id;
					pos_back[5 + i*3 + 1] = pos_data[5];
					pos_back[5 + i*3 + 2] = pos_data[6];
				}
				UART1SendDataPacket(pos_back,pos_back[2] + 2);
				break;
